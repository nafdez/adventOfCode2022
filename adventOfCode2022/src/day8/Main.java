	...
	public static void main(String[] args) {
		...
		for(int i = 0; i<=realVL; i++) {
			for(int j = 0; j<=realHL; j++) {
				int tree = forest[i][j];
				if(i==0 || j==0 || i==realVL || j==realHL || visibleLeft(forest, tree, i, j) || visibleRight(forest, tree, i, j, realHL) || visibleUp(forest, tree, i, j) || visibleDown(forest, tree, i, j, realVL))
					visibles++;
				countH++;
			}
			countV++;
